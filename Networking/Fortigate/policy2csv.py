...
To run:
Log into your firewall and backup your config file (save it a a plain
text file).  Edit this file and remove all lines above and including
"config firewall policy" and then remove all lines below and including
the first line that equals "end".  (this is really the end of the 'config
firewall policy' stanza).

So now your file should only contain a group of lines like this:
edit 123 ... next
edit 456 ... next
etc.
Save this file to policy.txt

Edit lines 73-74, srcint= and dstint= to match your firewall interface
names

With Python 2.6 or 2.7, run this command:
policy2csv.py > policy.tsv

Now you can load policy.tsv into excel.  Once your file is in excel,
highlight all columns and double-click the last column to expand them.
Do the same for rows, as mulitple sources & destinations are in the same
cell, but on different lines.
...
for rule in rule_set:
	...
	# Change these to your specific needs
	...
	if 0:  # to debug, change this to 1
		print rule_num
		print srcint + " -->> " +  dstint
